...
def main():
    # The pattern is ../mbt-fonts-*/*/moon.pkg
    # We want to find all moon.pkg files in the sibling directories.
    pattern = "../mbt-fonts-*/*/moon.pkg"
    paths = glob.glob(pattern)
    paths.sort()

    font_packages = []
    for path in paths:
        # Example path: ../mbt-fonts-a/airstream/moon.pkg
        # Normalize path to handle separators consistently.
        parts = path.replace('\\', '/').split('/')

        # We expect: [.., "mbt-fonts-x", "fontname", "moon.pkg"]
        if len(parts) >= 3:
            repo_dir = parts[-3]
            font_dir = parts[-2]
